...
const DeviceConfig DEVICE_CONFIGS[] = {
    // Building device configuration
    {
        ...
        .pins = {
            ...
            .pirPin = 18,            // D9, PIR motion sensor
            .pirPin2 = 10,           // D7, Second PIR sensor
    ...
    {
        .token = "WsyJtTftCGVWuGCnQ0OQ", .deviceType = DEVICE_TYPE_CARPARK, .deviceName = "Carpark_Management_System",
...
bool isFeatureEnabled(const char *feature)
{
    if (!currentConfig)
        return false;
    if (strcmp(feature, "temp_humidity") == 0)
        return currentConfig->enableTempHumidity;
    if (strcmp(feature, "air_quality") == 0)
        return currentConfig->enableAirQuality;
    if (strcmp(feature, "pir") == 0)
        return currentConfig->enablePIR;
    if (strcmp(feature, "lighting") == 0)
        return currentConfig->enableLighting;
    if (strcmp(feature, "rfid") == 0)
        return currentConfig->hasRFID;
    if (strcmp(feature, "ultrasonic") == 0)
        return currentConfig->hasUltrasonic;
